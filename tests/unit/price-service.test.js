...
describe('price service real data mapping', () => {
  ...
  it('calculates real savings only from provided prices', () => {
    ...
    expect(buildShareText(prices)).toBe('Encontre Aceite mas barato en Tienda Inglesa con AhorroYA. Ahorro estimado: $20. Probalo y compara antes de comprar: /app/buscar?q=aceite&utm_source=whatsapp&utm_medium=share&utm_campaign=montevideo_launch&store=Tienda+Inglesa&savings=20');
  });
  ...
  it('generates Montevideo launch content only from target supermarkets', () => {
    ...
    expect(content[0].whatsappText).toContain('mas barato en Devoto con AhorroYA. Ahorro estimado: $4.');
  });
});
