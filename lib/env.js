...
export const envSchema = z.object({
  ...
  AI_PROVIDER: z.enum(['mock', 'openai', 'gemini', 'vercel_gateway']).optional().default('mock'),
  AI_AUTONOMY_LEVEL: z.string().optional().default('LEVEL_0_READ_ONLY'),
  AI_GATEWAY_ENABLED: boolish.default(false),
  AI_GATEWAY_MODEL: z.string().optional().default('openai/gpt-5.5'),
  AI_GATEWAY_MAX_OUTPUT_TOKENS: z.coerce.number().int().min(1).max(4000).optional().default(600),
  AI_GATEWAY_SMOKE_PROMPT_ENABLED: boolish.default(false),
  AI_KILL_SWITCH: boolish.default(false),
  ENABLE_AI_AGENTS: boolish.default(false),
  ENABLE_AGENT_SCHEDULER: boolish.default(false),
  ENABLE_ADMIN_AI_PANEL: boolish.default(false),
  ENABLE_AI_LEVEL4_OVERRIDE: boolish.default(false),
...
export function readEnv(source = process.env) {
  const parsed = envSchema.safeParse({
    ...
    OPENAI_MODEL: source.OPENAI_MODEL || 'gpt-4.1-mini',
    GEMINI_API_KEY: source.GEMINI_API_KEY || '',
    GEMINI_MODEL: source.GEMINI_MODEL || 'gemini-2.5-flash',
    AI_PROVIDER: source.AI_PROVIDER || 'mock',
    AI_AUTONOMY_LEVEL: source.AI_AUTONOMY_LEVEL || 'LEVEL_0_READ_ONLY',
    AI_GATEWAY_ENABLED: source.AI_GATEWAY_ENABLED ?? 'false',
    AI_GATEWAY_MODEL: source.AI_GATEWAY_MODEL || 'openai/gpt-5.5',
    AI_GATEWAY_MAX_OUTPUT_TOKENS: source.AI_GATEWAY_MAX_OUTPUT_TOKENS || 600,
    AI_GATEWAY_SMOKE_PROMPT_ENABLED: source.AI_GATEWAY_SMOKE_PROMPT_ENABLED ?? 'false',
    AI_KILL_SWITCH: source.AI_KILL_SWITCH ?? 'false',
    ENABLE_AI_AGENTS: source.ENABLE_AI_AGENTS ?? 'false',
    ENABLE_AGENT_SCHEDULER: source.ENABLE_AGENT_SCHEDULER ?? 'false',
    ENABLE_ADMIN_AI_PANEL: source.ENABLE_ADMIN_AI_PANEL ?? 'false',
    ENABLE_AI_LEVEL4_OVERRIDE: source.ENABLE_AI_LEVEL4_OVERRIDE ?? 'false',
    TWILIO_ACCOUNT_SID: source.TWILIO_ACCOUNT_SID || '',
    TWILIO_AUTH_TOKEN: source.TWILIO_AUTH_TOKEN || '',
    TWILIO_WHATSAPP_NUMBER: source.TWILIO_WHATSAPP_NUMBER || '',
